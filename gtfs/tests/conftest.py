...
from gtfs.models import Departure, Route, Stop, StopTime, Trip
...
def route_with_departures(maas_operator, api_id_generator):
    ...
    route = baker.make(Route, feed=feed, api_id=api_id_generator)
    trips = baker.make(
        Trip,
        route=route,
        feed=feed,
    ...
    stops = baker.make(Stop, feed=feed, _quantity=2)
